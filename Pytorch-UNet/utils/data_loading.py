...
class GateDataset(BasicDataset):
    def __init__(self, images_dir, mask_fn, num_kp = 16):
        ...
        kp_list = []
        for elem in data:
            elem = [float(tmp) for tmp in elem]
            kp_list.append(elem)
    ...
    def __len__(self):
        return len(self.kp_list)

    def __getitem__(self, idx):
        # name = self.ids[idx]
        img_file = os.path.join(self.images_dir, f'img_{idx}.png')
        # assert len(img_file) == 1, f'Either no image or multiple images found for the ID {name}: {img_file}'
        img = load_image(img_file)

        processed_img = self.preprocess(img) 
        kp_vectors = self.kp_list[idx]
        hms = self._convert_to_hm(processed_img.shape[1], processed_img.shape[2], kp_vectors, sigma=2)*255
        # plt.figure(0)
        # plt.imshow(np.transpose(processed_img,axes=[1,2,0]))
        # plt.figure(1)
        # plt.imshow(np.sum(hms,axis=0))
        # plt.show()
        return {
            'image': torch.as_tensor(processed_img.copy()).float().contiguous(),
            'mask': torch.as_tensor(hms.copy()).float().contiguous()
        }
    ...
    def _convert_to_hm(self, H, W, keypoints, sigma=5):
        nKeypoints = len(keypoints)
...
if __name__ == "__main__":
    ...
    img_dir = os.path.join(script_dir, '../../baselines/img/')
    mask_fn = os.path.join(script_dir, '../../baselines/label/img.txt')

    loader = GateDataset(img_dir, mask_fn)
